...
extension Conversions {
  public struct MapValues<C: AsyncConversion>: AsyncConversion {
    var transform: C
...
extension Conversions {
  public struct MapKVPairs<KeyConversion: AsyncConversion, ValueConversion: AsyncConversion>: AsyncConversion where KeyConversion.Input: Hashable, KeyConversion.Output: Hashable {
...
extension Dictionary {
  ...
  ) async rethrows -> [NewKey: NewValue]  {
    return try await Dictionary<NewKey, NewValue>(uniqueKeysWithValues: self.concurrentMap {
      async let newKey = keyTransform($0.0)
      async let newValue = valueTransform($0.1)
